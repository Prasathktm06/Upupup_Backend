...
<div class="content-wrapper">
     <section class="content-header">
          <h1>Hot Offer Settings</h1>
          <div class="col-md-8" >
               <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'add_offer')) { ?>
            <a href="<?php echo base_url('hot_offer/add') ?>" class="btn btn-sm pull-right btn-warning" style="margin-top: -29px;margin-right: -230px;">Add Setting <i class="fa fa-plus"></i></a>
            <?php } ?>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td class="text-center" style="width: 15%"><?=$value['location']?></td>
                                                                 <td class="text-center">
                                                                    <?php if($value['dates']!=NULL){?>
                                                                        <?=date( ' d M Y ',strtotime($value['dates']))?>
                                                                    <?php }?>
                                                                </td>
                                                                <td class="text-center" style="width: 15%"><?=$value['percentage']?>%</td>
                                                               <td class="text-center" style="width: 15%">
                                                                  <?php if($value['status']==1){?>
                                                                   <a href="<?=base_url()?>hot_offer/change_status/<?=$value['id']?>/<?=$value['status']?>/<?=$value['location_id']?>" type="button" class="btn btn-success">Active</i></a>
                                                                  <?php } else{?>
                                                                            
                                                                    <a href="<?=base_url()?>hot_offer/change_status/<?=$value['id']?>/<?=$value['status']?>/<?=$value['location_id']?>" type="button" class="btn btn-danger">Inactive</i></a>
                                                                  <?php } ?>
                                                                </td>
                                                                <td class="text-center" style="width: 15%">
                                                                 <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'delete_hot_offer')) { ?>
                                                                      <a href="<?=base_url()?>hot_offer/delete_hotset/<?=$value['id']?>" class=" btn-small delete" title="Delete"><i class="fa fa-trash"></i> </a>
                                                                  <?php } ?>
                                                                </td>    
