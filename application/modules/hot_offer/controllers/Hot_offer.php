...
class Hot_offer extends CI_controller {
	   ...
	   if($this->input->post()){
		...
		$time2=$this->input->post('time2');
	    	...
	    	$data=array(
				...
				'time1'=>date('H:i:s',strtotime('+01 seconds',strtotime($time1))),
				'time2'=>date('H:i:s',strtotime('+01 seconds',strtotime($time2))),
				'status'=>$status
				);
	...
	public function change_not_status($id,$status,$location_id)
	{
		$active_seting=$this->Hotoffer_model->get_active_setting($location_id);
		foreach($active_seting as $row) {
             $active_id = $row->id;
             $insert_datas=array('status'=>0);
             $result=$this->Hotoffer_model->update($insert_datas,'hot_offer_notification',$active_id);
             }
        if($status==1){
		...
		// "<pre>";print_r($active_seting);exit();
	...
	//////////// change  status  /////////////////////////
	public function change_status($id,$status,$location_id)
	{
	    	$active_seting=$this->Hotoffer_model->get_active_hotsetting($location_id);
		foreach($active_seting as $row) {
             $active_id = $row->id;
             $insert_datas=array('status'=>0);
             $result=$this->Hotoffer_model->update($insert_datas,'hot_offer_setting',$active_id);
             }
		if($status==1){
		$insert_data=array('status'=>0);
		}else{
