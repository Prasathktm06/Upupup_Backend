...
<div class="content-wrapper">
     <section class="content-header">
          ...
          <div class="col-md-8" >
            ...
            <a href="<?php echo base_url('trainers/add') ?>" class="btn btn-sm pull-right btn-warning" style="margin-top: -29px;margin-right: -230px;">Add Trainers & Coaches <i class="fa fa-plus"></i></a>
            <?php } ?>
                                          ...
                                          <?php $i=1; foreach ($list as $key => $value) { ?>
                                                            <tr>
                                                                ...
                                                                <td class="text-center" style="width: 15%">
                                                                  <?php if(!empty($value['profile_image'])){?> <img src="<?=$value['profile_image']?>" class="img-circle" width="100px" height="100px"> <?php }else{ ?> <img src="<?=base_url()?>assets/img/user_default.png" class="img-circle" width="100px" height="100px">  <?php }?></td>
                                                                ...
                                                                <td class="text-center" style="width: 15%">
                                                                      <?php if($value['status']==1){?>
                                                                          <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'edit_trainers')) { ?>
                                                                                <a href="<?=base_url()?>trainers/change_status/<?=$value['id']?>/<?=$value['status']?>" type="button" class="btn btn-success">Active</i></a>
                                                                          <?php }?>
                                                                      <?php } else{?>
                                                                          <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'edit_trainers')) { ?>   
                                                                                <a href="<?=base_url()?>trainers/change_status/<?=$value['id']?>/<?=$value['status']?>" type="button" class="btn btn-danger">Inactive</i></a>
                                                                          <?php }?>
                                                                      <?php } ?>
                                                                </td>
                                                                <td class="text-center" style="width: 15%">
                                                                  <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'edit_trainers')) { ?>
                                                                      <a href="<?=base_url()?>trainers/trainers_edit/<?=$value['id']?>" class=" btn btn-small" title="Edit"><i class="fa fa-pencil"></i></a>
                                                                  <?php } ?>
                                                                  <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'delete_trainers')) { ?>
                                                                      <a href="<?=base_url()?>trainers/delete_trainer/<?=$value['id']?>" class=" btn-small delete" title="Delete"><i class="fa fa-trash"></i> </a>
                                                                  <?php } ?>
                                                                </td>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form role="form" method="post" action="<?=base_url();?>trainers">
                            <div class="box-body">
