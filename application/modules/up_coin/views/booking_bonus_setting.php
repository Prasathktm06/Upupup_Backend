     ...
     <section class="content-header">
        ...
        <div class="col-md-6" >
             ...
             <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'add_up_coin')) { ?>
                                    <a href="<?php echo base_url('up_coin/add_booking_bonus') ?>" class="btn btn-sm pull-right btn-warning" style="margin-top: -29px;margin-right: -200px;">Add Setting <i class="fa fa-plus"></i></a>
                                    <?php }?>
                                                               ...
                                                               <td class="text-center" style="width: 15%">
                                                                  <?php if($value['status']==1){?>
                                                                        <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'edit_up_coin')) { ?>
                                                                            <a href="<?=base_url()?>up_coin/change_booking_bonus_status/<?=$value['id']?>/<?=$value['status']?>" type="button" class="btn btn-success">Active</i></a>
                                                                         <?php }?>
                                                                  <?php } else{?>
                                                                        <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'edit_up_coin')) { ?>     
                                                                            <a href="<?=base_url()?>up_coin/change_booking_bonus_status/<?=$value['id']?>/<?=$value['status']?>" type="button" class="btn btn-danger">Inactive</i></a>
                                                                         <?php }?>
                                                                  <?php } ?>
                                                                </td>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form role="form" method="post" action="<?=base_url();?>up_coin/booking_bonus">
                            <div class="box-body">
