...
<div class="content-wrapper">
     <section class="content-header">
          ...
          <div class="col-md-8" >
            ...
            <a href="<?php echo base_url('advertisement/add_trainer_adv') ?>" class="btn btn-sm pull-right btn-warning" style="margin-top: -29px;margin-right: -230px;">Add Trainer Advertisement 1 <i class="fa fa-plus"></i></a>
            <?php } ?>
                                                                ...
                                                                <td class="text-center">
                                                                      <?php if($value['status']==1){?>
                                                                          <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'edit_advertisement')) { ?>
                                                                                <a href="<?=base_url()?>advertisement/trainer_adv_status/<?=$value['id']?>/<?=$value['status']?>" type="button" class="btn btn-success">Active</i></a>
                                                                          <?php }?>
                                                                      <?php } else{?>
                                                                          <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'edit_advertisement')) { ?>   
                                                                                <a href="<?=base_url()?>advertisement/trainer_adv_status/<?=$value['id']?>/<?=$value['status']?>" type="button" class="btn btn-danger">Inactive</i></a>
                                                                          <?php }?>
                                                                      <?php } ?>
                                                                </td>
                                                                <td class="text-center" style="width: 15%">
                                                                  <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'edit_advertisement')) { ?>
                                                                      <a href="<?=base_url()?>advertisement/edit_trainer_adv/<?=$value['id']?>" class=" btn btn-small" title="Edit"><i class="fa fa-pencil"></i></a>
                                                                  <?php } ?>
                                                                  <?php if($this->acl_model->user_has_perm($this->session->userdata('user_id'), 'delete_advertisement')) { ?>
                                                                      <a href="<?=base_url()?>advertisement/delete_trainer_adv/<?=$value['id']?>" class=" btn-small delete" title="Delete"><i class="fa fa-trash"></i> </a>
                                                                  <?php } ?>
                                                                </td>
