...
<div class="row" style="border-style: solid;border-color: black;border-width: 5px;margin: 10%;margin-top: 0;font-family: 'Copperplate / Copperplate Gothic Light, sans-serif';" >
	...
	<div class="col-md-12 text-center">
		<center>
		<p style=""> <?=$data['user']['name']?> (Mob: <?=$data['user']['phone_no']?>)<?php if($data['co_players']!='') echo ','.$data['co_players']; ?> .</p>
    <h3 style="">New booking is confirmed!</h3>
    <?php if($data['Coplayer']!=''){ ?>
    <h6 style="color:#500050;font-size:x-small;"><?=$data['user']['name']?> has booked a venue and added you as a coplayer :)</h6>
    <?php }else{
        if($data['co_players']!=''){?>
          <h6 style="color:#500050;font-size:x-small;"><?=$data['user']['name']?> has booked a venue and added <?=$data['co_players']?> as coplayer. </h6>
          <?php }else{ ?>
        <h6 style="color:#500050;font-size:x-small;"><?=$data['user']['name']?> has booked a venue . </h6>
  <?php  }} ?>
